...
class DebugOverlayService : Service() {
    ...
    private fun showNotification() {
        ...
        overlayViewManager?.let { manager ->
            if (manager.isSystemOverlayShown()) {
                builder.addAction(
    ...
    private fun getBroadcastIntent(action: String): PendingIntent {
        ...
        return PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_CANCEL_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    }

    private fun getLaunchIntent(): PendingIntent? {
        return config.activityName?.let { className ->
            try {
                ...
                Log.w(TAG, "${config.activityName} was not found - ${ne.message}")
                null
            }
        }
    }
    ...
    companion object {
        ...
        fun createIntent(context: Context): Intent {
            return Intent(context, DebugOverlayService::class.java)
        }
        ...
        private fun getAppIcon(context: Context): Bitmap? {
            val drawable: Drawable? = try {
                ...
                Log.w(TAG, "Package Not found:${context.packageName}")
                null
            }
